...
const handleRequest = async (request) => {
  const responseType = await isJsonFormat(request);
...
const isJsonFormat = async (request) => {
  const { searchParams } = new URL(request.url);

  let format = 'json';

  if (searchParams && searchParams.get("format")) {
      format = searchParams.get("format").toLocaleLowerCase().split("/")[0];
  }

  const accept = request.headers.get('Accept');

  return accept != 'text/plain' && format != 'text';
};
